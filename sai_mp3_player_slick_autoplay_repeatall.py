...
class TTSAnnouncer:
    """
    Reliable Windows text-to-speech using a PowerShell SAPI call.

    Why this approach:
    - pyttsx3 on Windows can speak once and then stop (thread/COM issues).
    - Spawning a short PowerShell SAPI speak per announcement is extremely
      reliable and keeps the GUI responsive.

    This announcer runs a single worker thread and serializes announcements.
    """
    ...
    def say(self, phrase: str) -> None:
        ...
        # Keep only the latest request to match "current song"
        self._drain_queue()
        self._q.put(phrase)
    ...
    def _powershell_speak(self, phrase: str) -> None:
        ...
        # -NoProfile for speed, hide window, and capture output
        ...
        # Wait (blocks worker thread only)
        try:
            self._ps_proc.wait(timeout=15)
        except Exception:
            self._kill_running()
    ...
    def _run_worker(self) -> None:
        while not self._stop_event.is_set():
            ...
            if sys.platform.startswith("win"):
                ...
                # Non-Windows: no-op (or add other OS support later)
                pass
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # - When adding/loading tracks for the first time, start playback.
        self.auto_play = True
    ...
    def _announce_track_name(self, track: Track) -> None:
        self._tts.say(Path(track.path).stem)
