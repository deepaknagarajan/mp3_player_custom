...
QComboBox QAbstractItemView {
    background: #1E1230;
    border: 1px solid #4C1D95;
    selection-background-color: #2B1450;
    selection-color: #EAEAEA;
    outline: 0;
}
...
DEFAULT_THEME = "Dark Blue (Default)"
...
def _get_windows_voices() -> list[str]:
    """
    Return installed Windows SAPI voices via PowerShell.
    """
...
def _get_macos_voices() -> list[str]:
    """
    Return installed macOS voices using: say -v ?
    """
...
def _get_tts_voices() -> list[str]:
    if sys.platform.startswith("win"):
        return _get_windows_voices()
    if sys.platform == "darwin":
        return _get_macos_voices()
    return []
...
class TTSAnnouncer:
    def __init__(self) -> None:
        ...
        # We reuse the same attribute name that previously held a PowerShell proc.
        # On macOS it will hold the 'say' process.
    ...
    def _macos_speak(self, phrase: str) -> None:
        cmd: list[str] = ["say"]
        if self._voice_name:
            cmd += ["-v", self._voice_name]
        cmd.append(phrase)
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.voice_combo.setToolTip("Choose text-to-speech voice (Windows/macOS)")
        self.voice_combo.addItem("Default Voice", "")
        for v in _get_tts_voices():
            self.voice_combo.addItem(v, v)
        self.voice_combo.currentIndexChanged.connect(self._on_voice_changed)

        self.theme_combo = QComboBox()
        self.theme_combo.setToolTip("Choose app color theme")
    ...
    def _apply_theme(self, name: str) -> None:
        ...
        # Force refresh by briefly clearing then re-applying.
        app.setStyleSheet("")
        app.setStyleSheet(qss)

        # Re-polish widgets so changes apply immediately.
        try:
...
def main() -> int:
    ...
    app.setStyleSheet(THEMES.get(DEFAULT_THEME, ""))
    win = MainWindow()
    win.show()
    return app.exec()
