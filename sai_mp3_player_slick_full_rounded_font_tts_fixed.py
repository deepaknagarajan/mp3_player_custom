...
APP_QSS = """
QWidget {
    /* Rounded-ish font stack (uses what is available on Windows) */
...
def _setup_vlc_dll_paths() -> str | None:
    """
    Make VLC DLLs visible to this process before importing python-vlc.
    Returns the VLC directory if found, else None.
    """
    candidates = [
...
class TTSAnnouncer:
    """
    A safe pyttsx3 wrapper for Windows.

    Key detail: On Windows, pyttsx3/SAPI tends to behave poorly if you call
    runAndWait() from many different threads or from a different thread than
    initialization. This class initializes pyttsx3 inside ONE worker thread
    and processes a queue of phrases to speak.
    """
    ...
    def say(self, phrase: str) -> None:
        ...
        if not phrase:
            return
        # Clear any backlog so the announcement always matches the current song.
        self._drain_queue()
        self._q.put(phrase)

    def shutdown(self) -> None:
        self._stop_event.set()
        self._q.put("")  # wake worker
    ...
    def _run_worker(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                # If the engine gets into a bad state, try to re-init once.
                try:
                    engine = pyttsx3.init()
                    engine.setProperty("rate", self._rate)
                    engine.setProperty("volume", self._volume)
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # Text-to-speech announcer (thread-safe)
        self._tts = TTSAnnouncer(rate=175, volume=1.0)
        ...
        # Highlight state
        self._playing_track_index: int = -1
    ...
    def _tick(self) -> None:
        ...
        if length > 0:
            ...
            ratio = pos / float(length) if length > 0 else 0.0
